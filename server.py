...
def verificar_faturas():
    ...
    try:
        ...
        if data.get("totalCount") == 0:
            return (
                jsonify({"status": "erro", "mensagem": "Aluno não encontrado."}),
                404,
            )
...
def relatorio_polo_historico():
    ...
    try:
        ...
        if not polo:
            return (
                jsonify({"status": "erro", "mensagem": "Campo obrigatório: polo"}),
                400,
            )
